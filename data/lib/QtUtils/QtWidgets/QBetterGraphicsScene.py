...
from PySide6.QtCore import QPoint, QPointF, Qt, QRect, QRectF
from PySide6.QtGui import QPen, QBrush, QFont, QPixmap, QImage, QPainterPath, QPolygon, QPolygonF
from PySide6.QtWidgets import QGraphicsScene, QGraphicsItem, QGraphicsEllipseItem, QGraphicsRectItem, QGraphicsTextItem, QGraphicsPixmapItem, QGraphicsPathItem, QGraphicsLineItem, QGraphicsPolygonItem, QGraphicsSimpleTextItem, QGraphicsItemGroup, QGraphicsProxyWidget, QWidget, QGraphicsView
...
class QBetterGraphicsScene(QGraphicsScene):
    def __init__(self, parent: QGraphicsView, size: QRect | QRectF) -> None:
    ...
    def items_at(self, pos: QPoint | QPointF) -> list[QGraphicsItem | QBetterGraphicsItemWidget]:
        items = self.items(pos, Qt.ItemSelectionMode.ContainsItemBoundingRect, Qt.SortOrder.AscendingOrder)
        return [item for item in items if item.isVisible() and item.flags() & QGraphicsItem.GraphicsItemFlag.ItemIsSelectable]
    ...
    def add_item(self, item: QGraphicsItem | QBetterGraphicsItemWidget, layer: int = 0) -> QGraphicsItem | QGraphicsProxyWidget:
        ...
        if isinstance(item, QBetterGraphicsItemWidget):
            ...
            proxy.setFlags(QGraphicsItem.GraphicsItemFlag.ItemIsMovable | QGraphicsItem.GraphicsItemFlag.ItemIsSelectable | QGraphicsItem.GraphicsItemFlag.ItemIsFocusable)
            return proxy

        self._layers[layer].add_item(item)
        if item.scene() != self: super().addItem(item)
