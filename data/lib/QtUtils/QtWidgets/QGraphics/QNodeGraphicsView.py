...
from .. import QBetterGraphicsView
...
class QNodeGraphicsView(QBetterGraphicsView):
    ...
    def __init__(self, size: QRect | QRectF) -> None:
            ...
            if isinstance(item, QNodeGraphicsItemWidget):
                for field in item.fields:
                    ...
                    if conn.connection_type == goal_type:
                        half_size = QPointF(conn.size().width() / 2, conn.size().height() / 2)
                        rect = QRectF(
                            self._fix_position(conn.global_position) - half_size,
                            self._fix_position(conn.global_position) + half_size
                        )

                        if rect.contains(pos) and self._current_connector.can_connect(conn) and conn.can_connect(self._current_connector):
