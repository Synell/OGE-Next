...
class QNodeGraphicsItemWidgetField(QGridFrame):
    ...
    class Connector(QGridFrame):
        ...
        def global_position(self) -> QPoint:
            half = QPoint(self._connect_point.width() // 2, self._connect_point.height() // 2)

            root = self
            while root.parentWidget(): root = root.parentWidget()
            proxy = root.graphicsProxyWidget()

            root_pos = self.mapTo(root, self._connect_point.pos() + half)
            scene_pos = proxy.mapToScene(root_pos)

            view = proxy.scene().views()[0]
            view_pos = view.mapFromScene(scene_pos)

            return view.mapToGlobal(view_pos)
