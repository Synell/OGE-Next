...
class GradeWidget(OGEWidget):
    ...
    def __init__(self, grade: Grade) -> None:
        ...
        label = QLabel(f'{grade.rank if grade.rank is not None else "?"}/{grade.rank_total if grade.rank_total is not None else "?"}')
        self.setToolTip(label)

        label = QLabel(f'{grade.value if grade.value is not None else "?"}/{grade.total}')
